...
const TIPO_COLORS = {
  'Cumpleaños':      { bg: 'rgba(168,85,247,.15)', color: 'var(--purple)' },
...
export default function LeadsSheet({ leads, onSync, onDelete, syncing }) {
  ...
  // Tipos únicos
  const tipos = [...new Set(leads.map(l => l.tipo_evento).filter(Boolean))]

  const estadoCounts = {}
  ESTADOS.forEach(e => { estadoCounts[e] = leads.filter(l => l.estado === e).length })
  ...
  return (
    <section className="view">
      <div className="page-head">
        <div>
          <h1>Leads</h1>
          <p>Prospectos sincronizados desde Google Sheets · {leads.length} registros</p>
        </div>
        <button
            ...
            {rows.length === 0 ? (
              <tr>
                <td colSpan={10} className="empty">
                  ...
                  {leads.length === 0
                    ? 'No hay leads sincronizados aún. Haz clic en "Sincronizar Sheets".'
                    : 'No hay leads que coincidan con los filtros.'}
                </td>
              </tr>
            ) : rows.map(l => {
              const tipoStyle = TIPO_COLORS[l.tipo_evento] || { bg: 'rgba(140,108,255,.1)', color: 'var(--muted)' }
              return (
                <tr key={l.id}>
